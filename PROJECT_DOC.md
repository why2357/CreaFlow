...
**创流 (Creaflow)** 是一个基于 Vue 3 + TypeScript + Element Plus + Vite 的 AI 视频创作工作台前端应用，负责用户交互、AI
内容生成操作与项目历史记录管理。
...
**各分支职责说明：** | 分支类型 | 命名规范 | 用途 | 合并目标 | |----------|----------|------|----------| | `main` | - |
生产环境稳定分支 | 仅接受 `develop` 或 `hotfix/*` 合并 | | `develop` | - | 开发主分支，日常开发 | 合并至 `main` | |
`feature/*` | `feature/功能描述` | 新功能开发 | 合并至 `develop` | | `hotfix/*` | `hotfix/问题描述` | 线上紧急修复 | 同
时合并至 `main` 和 `develop` |
