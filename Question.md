...
## 2026-02-13: 开发环境登录后显示 404 页面

### 问题描述

开发环境登录成功后显示 404 页面，而生产环境正常工作。
...
在开发环境中，如果后端 `/hivision/getRouters` 返回空数组，加上静态菜单没有注册，导致没有任何可用路由，最终匹配到 404
catch-all 路由。
...
编写一个脚本，用于将打包好的 `dist` 目录中的文件上传到内网服务器的 `/opt/1panel/www/sites/creaflow/index`，并在部署前自
动备份服务器上的现有文件。
...
在 `@` 后继续输入字符时，这些字符会自动被当作搜索关键词，同时 MentionDropdown 的搜索框会自动聚焦，导致用户无法正常在
textarea 中输入。
...
// 替换@符号及其后的文本
const newValue = currentValue.substring(0, triggerPos) + insertText + currentValue.substring(cursorPosition);
...
## 2026-02-27: 修复参考图上传后不显示的问题

### 问题描述

在 SeedancePromptEditor 组件中，上传参考图后图片数据被保存到 `referenceStore.images`，但 UI 上没有显示已上传的图片。

### 原因分析

1. **父组件传入 `showReferenceBar="false"`**：在 `StoryboardTable.vue` 中显式设置了 `:show-reference-bar="false"`

2. **第一个参考图显示区域的渲染条件**：
   ```vue
   v-if="showReferenceBar && referenceStore.images.length > 0"
   ```
   由于 `showReferenceBar` 为 false，这个区域永远不会显示

3. **input-actions 区域只显示上传按钮**：
   该区域的 `reference-images-section` 内只包含一个 `el-upload` 组件（上传按钮），没有渲染 `referenceStore.images` 列表

4. **input-wrapper 的显示条件**：
   ```vue
   v-if="!showReferenceBar || referenceStore.images.length === 0"
   ```
   当 `showReferenceBar=false` 时永远显示，但里面的上传区域不显示已上传的图片

### 解决方案

在 `input-actions` 的 `reference-images-section` 中添加已上传图片的渲染逻辑：

```vue
<div class="input-actions">
  <div class="reference-images-section" :class="{ 'has-images': referenceStore.images.length > 0 }">
    <div class="reference-images-stack" :class="{ 'is-expanded': isHoveringImages }">
      <TransitionGroup name="stack-slide">
        <!-- 已上传的参考图 -->
        <div v-for="(image, index) in referenceStore.images" :key="image.id" ...>
          <img :src="image.src" class="reference-thumbnail" />
          <!-- 删除按钮 -->
        </div>

        <!-- 添加按钮 -->
        <div v-if="referenceStore.images.length < 3" ...>
          <el-upload>...</el-upload>
        </div>
      </TransitionGroup>
    </div>
...
同时添加样式，让有图片时区域宽度自动展开：

```scss
.reference-images-section {
  &.has-images {
    width: 180px; // 3张图片展开后的宽度
  }
}
```
...
- `src/components/TiptapEditor/SeedancePromptEditor.vue` - 添加图片渲染和展开样式
- `src/views/workbench/project-creation/steps/StepShotList/components/StoryboardTable.vue` - 父组件传入 `:show-reference-bar="false"`

---

## 2026-02-27: 修复上传多张图片后按钮被遮挡的问题

### 问题描述

上传3张图片后，上传按钮被遮挡或位置不正确。

### 原因分析

1. **数量限制过小**：代码限制最多3张图片 (`referenceStore.images.length < 3`)
2. **位置数组不够**：`getImageStackStyle` 和 `getAddButtonStyle` 的位置数组只有3个元素
3. **容器宽度不够**：展开后宽度只有 180px (3 * 60px)，无法容纳更多图片
4. **第一个区域没有展开样式**：顶部参考图区域缺少 `has-images` 样式来展开宽度

### 解决方案

1. **增加数量限制**：从 3 改为 5，支持最多5张图片（与即梦AI一致）
2. **扩展位置数组**：支持5张图片的位置计算
3. **增加容器宽度**：`has-images` 宽度从 180px 改为 300px (5 * 60px)
4. **添加展开样式**：给第一个参考图区域也添加 `has-images` class

### 相关文件

- `src/components/TiptapEditor/SeedancePromptEditor.vue` - 修复数量限制、位置计算和样式
